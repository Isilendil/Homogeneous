...
#   run.time: time consumed by this algorithm errors
...
HomOTL1.K.M <- function(Y, K1, K2, options, id.list, classifier)
{
  ...
  for (t in 1 : (length(ID)) )
  {
    ...
    ell1 <- (hat.f1 - (Y[id]+1)/2) ^ 2
    ell2 <- (hat.f2 - (Y[id]+1)/2) ^ 2
    w.1t <- w.1t * exp(-eta * ell1)
    w.2t <- w.2t * exp(-eta * ell2)
    sum.w <- w.1t + w.2t
    w.1t <- w.1t / sum.w
    w.2t <- w.2t / sum.w
